...
export class AuthService {
  ...
  startOAuth(provider: string, redirectUri: string): Observable<{ url: string }> {
    return this.http.post<{ url: string }>(`${OAUTH_AUTHORIZE_BASE}/start`, {
      provider,
      redirect_uri: redirectUri,
    });
  }

  handleOAuthCallback(params: { code?: string; state?: string; error?: string }): Observable<AuthResponse> {
    const httpParams = new HttpParams({ fromObject: params as Record<string, string> });
    return this.http.get<AuthResponse>(`${OAUTH_AUTHORIZE_BASE}/callback`, { params: httpParams });
  }
