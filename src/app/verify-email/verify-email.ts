    ...
    this.service
      ...
      .subscribe(
        (res: VerifyResult) => {
          ...
          const timeout = setTimeout(() => {
            if (res.success) {
              ...
              this.message =
                res.message ?? 'Verification failed. The token may be invalid or expired.';
            }
          }, 800);
          this.animationTimeouts.push(timeout);
        },
        (err) => {
          ...
          const timeout = setTimeout(() => {
            this.state = VerifyState.Error;
            this.message = 'Network error while verifying. Please check your connection and try again.';
          }, 500);
          this.animationTimeouts.push(timeout);
        }
      );
